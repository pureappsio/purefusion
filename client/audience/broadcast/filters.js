...
Template.filters.events({
    ...
    'change .select-criteria': function(event, template) {
        ...
        // Get brand ID
        var brandId = Session.get('selectedBrand');
        ...
        if (criteria == 'boughtproduct' || criteria == 'notboughtproduct') {
            ...
            Meteor.call('getProducts', Session.get('selectedBrand'), function(err, products) {
        ...
        if (criteria == 'interested') {
            ...
            // Get tags
            var interests = Tags.find({ brandId: Session.get('selectedBrand') }).fetch();
        ...
        if (criteria == 'received' || criteria == 'notreceived' || criteria == 'clickedbroadcast' || criteria == 'notclickedbroadcast' || criteria == 'openedbroadcast' || criteria == 'notopenedbroadcast') {
            ...
            // Get broadcasts
            var broadcasts = Broadcasts.find({ brandId: Session.get('selectedBrand') }).fetch();
    ...
    'click #select': function() {
        ...
        // Filter subscribers
        Meteor.call('getNumberFilteredSubscribers', Session.get('selectedBrand'), filters, function(err, filteredCustomers) {
